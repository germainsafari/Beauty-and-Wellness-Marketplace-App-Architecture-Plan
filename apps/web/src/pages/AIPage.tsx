...
export default function AIPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const session =
          sessions[0] ??
          (await trpcCall<{ id: number }>("ai.createSession", { locale }, "mutation"));
  ...
  const send = async (text?: string) => {
    ...
    try {
      const { reply } = await trpcCall<{ reply: string }>(
        "ai.chat",
      ...
      setMessages((prev) => [
        ...prev,
        { id: Date.now() + 1, role: "assistant", content: e instanceof Error ? e.message : "AI unavailable" },
      ]);
    } finally {
      setSending(false);
    }
    ...
    <div className="flex flex-col h-[calc(100vh-5rem)]">
      <div className="bg-gradient-to-r from-hafi-dark to-hafi-mid text-white p-4">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="font-black font-display">{t("ai.title")}</h1>
            <p className="text-purple-200 text-xs">{t("ai.subtitle")}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 p-4">
          {PROMPT_KEYS.map((key) => (
            <button
              key={key}
              onClick={() => send(t(key))}
              className="text-xs bg-white border border-purple-100 text-hafi-purple font-semibold px-3 py-2 rounded-full hover:bg-purple-50"
            >
              {t(key)}
            </button>
          ))}
        ...
        {messages.map((m) => (
          <div
            ...
            className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${
              m.role === "user" ? "ml-auto bg-hafi-purple text-white" : "bg-white shadow-sm"
            }`}
          >
            {m.content}
          </div>
      ...
      <div className="p-4 bg-white border-t border-purple-100 flex gap-2">
        ...
        <button
          onClick={() => send()}
          disabled={sending}
          className="w-12 h-12 bg-hafi-purple text-white rounded-full flex items-center justify-center disabled:opacity-50"
        >
